...
public:
    ...
    void insert(const iterator &it, const std::pair<K, V> &key_value) {
        ...
        //finds if there is a duplicate key being insterted
        if(find(key) != end()){
                cerr << "Warning: Attempt to insert duplicate key '" << key <<"'." <<endl;
                return;
            }
        //moves pointer to correct root position 
        while(x != nullptr){
        ...
        //right
            ...
            z->right = nullptr;
            z->color = RED;
            insert_fixup(z); //fixups the now newly inserted node in the tree
            size_ += 1;
...
private:
    ...
    int diameter(Node<K, V> *node) const {
        ...
        int diametermax = max(leftd,rightd);
        return max(lefth + righth + 1, diametermax);
    ...
    size_t sum_levels(Node<K, V> *node, size_t level) const {
        ...
        size_t sumlevels = sum_levels(node->left, level+1) + sum_levels(node->right, level+1) + level;
    ...
    size_t sum_null_levels(Node<K, V> *node, size_t level) const {
        ...
        size_t sumlevels = sum_null_levels(node->left, level + 1) + sum_null_levels(node->right,level + 1);
        return sumlevels;
        // TODO
    }
};
