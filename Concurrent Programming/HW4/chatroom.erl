...
%% sends message to each client on server
propegate_helper(Ref, Nick, ChatName, Message, List) ->
	if  List == [] -> ok;
		true ->
			[H | T] = List,
			H!{request, self(),Ref, {incoming_msg, Nick, ChatName, Message}},
			propegate_helper(Ref, Nick, ChatName, Message, T)
	end.

%% This function should update all clients in chatroom with new message
do_propegate_message(State, Ref, ClientPID, Message) ->
	ClientPID!{self(), Ref, ack_msg},
	propegate_helper(Ref, maps:get(ClientPID,State#chat_st.registrations), State#chat_st.name, Message, lists:delete(ClientPID,maps:keys(State#chat_st.registrations))),
	#chat_st{
		name = State#chat_st.name,
		registrations = State#chat_st.registrations,
		history = State#chat_st.history ++ [{maps:get(ClientPID,State#chat_st.registrations), Message}]
	}.