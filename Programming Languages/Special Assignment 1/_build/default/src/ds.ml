...
(*helper function that references each other to implement/do mutual recursion*)
let rec ae_helper =
  fun id decs ->
  match decs with
  | [] -> []
...
(*check if regular rec is applicable, otherwise utilize helper function to break down and use mutual recursion*)
let rec apply_env : string -> exp_val ea_result =
  ...
  | ExtendEnvRec(decs,tail) ->
    let help = ae_helper id decs in
    match help with
    | [] -> apply_env id tail
    | [x, y] -> Ok (ProcVal(x, y, env))
