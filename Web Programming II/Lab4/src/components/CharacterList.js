...
const CharacterList = (props) => {
	...
	useEffect(() => {
		...
		baseUrl = 'https://gateway.marvel.com:443/v1/public/characters?offset='+offset;
		...
		() => {
			...
			async function fetchData() {
				try {
					...
					baseUrl = 'https://gateway.marvel.com:443/v1/public/characters?nameStartsWith='+searchTerm;
					url = baseUrl + '&ts=' + ts + '&apikey=' + publickey + '&hash=' + hash;
					const { data } = await axios.get(url);
					setSearchData(data);
					setLoading(false);
	...
	const NextPage = () => {
		...
		return(
			<Link to={`/Characters/page/${nextPage}`}>Next Page</Link>
		);
	};
	...
	const PrevPage = () => {
		...
		return(
			<Link to={`/Characters/page/${prevPage}`}>Previous Page</Link>
		);
	};
	...
	if(loading){
		...
		let totalPages = Math.floor(CharacterData.data.total / CharacterData.data.limit)
		if((CharacterData.data.count === 0) || (isNaN(parseInt(props.match.params.page)))){
			return( <Redirect to ="/ErrorPage" />)
		}
