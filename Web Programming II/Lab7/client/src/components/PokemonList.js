...
const PokemonList = (props) => {
	...
	useEffect(() => {
		async function fetchData() {
			try {
				const {data} = await axios.get('http://localhost:3000/pokemon/page/' + props.match.params.page)
				console.log(data)
				setPokemonData(data)
				setLoading(false)
			} catch (e) {
	...
	if(loading){
		...
		if(pokemonData){
			...
			pokemonData.results.map((pokemon) => {
				...
				let spriteUrl = 'https://raw.githubusercontent.com/PokeAPI/sprites/master/sprites/pokemon/'+id.toString() +'.png'
			...
			if(pokemonData.next){
				NextPage = <Link to={`/pokemon/page/${parseInt(props.match.params.page) + 1}`}>Next Page</Link>
			}
			if(pokemonData.previous){
				PrevPage = <Link to={`/pokemon/page/${parseInt(props.match.params.page) - 1}`}>Previous Page</Link>
			}
